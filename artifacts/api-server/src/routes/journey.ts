...
const stageTemplates = [
  {
    ...
    duration: "Months 1-2",
    description: "Understand the core language, tools, and vocabulary of the target field.",
    resources: [
      { name: "Kaggle Python micro-course", type: "free" as const, price: "GBP 0" },
      { name: "Coursera foundations audit", type: "free" as const, price: "GBP 0" },
      { name: "Role-specific reference book", type: "paid" as const, price: "GBP 28" },
    ],
    checklist: [
      { key: "starter-course", title: "Finish the starter course", completed: false },
      { key: "notes-repo", title: "Create a public notes repository", completed: false },
      { key: "small-project", title: "Build one small portfolio example", completed: false },
    ],
  },
  {
    stageOrder: 2,
  ...
  },
  {
    ...
    resources: [
      { name: "Interview practice checklist", type: "free" as const, price: "GBP 0" },
      { name: "Target-role profile review", type: "free" as const, price: "GBP 0" },
      { name: "One-to-one advisor session", type: "paid" as const, price: "GBP 30" },
    ],
    checklist: [
      { key: "applications", title: "Apply to 10 realistic roles", completed: false },
      { key: "advisor-review", title: "Book an advisor review", completed: false },
      { key: "practice-interviews", title: "Record two practice interviews", completed: false },
    ],
  },
];

router.post("/journey/generate", requireAuth, async (req, res): Promise<void> => {
  ...
  if (!description && !profile?.professionalSummary && !goal?.targetRole) {
    res.status(400).json({ error: "Add a career description, profile summary, or career goal first." });
    return;
  }
...
router.post("/journey/build", requireAuth, async (req, res): Promise<void> => {
  const userId = req.user!.userId;
  const directionId = typeof req.body?.selectedDirectionId === "string"
  ...
  const selectedDirection = directionId === "career-goal" && goal?.targetRole
    ? {
        id: "career-goal",
        title: goal.targetRole,
        durationMonths: Math.max(6, Math.min(60, (goal.targetYears ?? 1) * 12)),
  ...
  const result = await db.transaction(async (tx) => {
    ...
    const [journey] = await tx
      ...
      .values({
        ...
        generatedFrom: analysis ? "career-analysis" : goal ? "career-goal" : "journey-builder",
      })
      .returning();
    ...
    await tx
      ...
      .values({ userId, journeyId: journey.id, frequency: "weekly", dayOfWeek: 1, contentLog: [] })
      .onConflictDoUpdate({
        target: weeklyRemindersTable.userId,
        set: { journeyId: journey.id, frequency: "weekly", dayOfWeek: 1, updatedAt: new Date() },
      });
...
router.patch("/journey/stage/:id/milestone", requireAuth, async (req, res): Promise<void> => {
  const stageId = Number(req.params.id);
  const checklistItemKey = typeof req.body?.checklistItemKey === "string"
    ? req.body.checklistItemKey.trim()
    : "";
  const completed = req.body?.completed !== false;

  if (!Number.isInteger(stageId) || stageId <= 0 || !checklistItemKey) {
    res.status(400).json({ error: "A valid stage id and checklistItemKey are required." });
    return;
  }
  ...
  const [stage] = await db
    .select()
    .from(journeyStagesTable)
    .where(and(eq(journeyStagesTable.id, stageId), eq(journeyStagesTable.userId, req.user!.userId)));
  if (!stage) {
    res.status(404).json({ error: "Journey stage not found." });
    return;
  }

  const itemExists = stage.checklist.some((item) => item.key === checklistItemKey);
  if (!itemExists) {
  ...
  await db.transaction(async (tx) => {
    ...
    const updatedStages = stages.map((item) =>
      item.id === stage.id ? { ...item, checklist, completed: stageCompleted } : item,
    );
